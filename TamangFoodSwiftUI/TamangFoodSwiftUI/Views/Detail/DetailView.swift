...
struct DetailView: View {
                    ...
                    VStack {
                        ...
                        LazyVStack {
                            ForEach(viewModel.listMealByCategory, id: \.idMeal) { item in
                                ListCategoryFoodView(screenSize: screenSize, action: {
                                    updateDataMeal(idMeal: item.idMeal, scrollProxy: scrollProxy)
                                }, viewModel: ListCategoryFoodViewModel(mealDB: item))
                            }
                        }
                        .onAppear {
                            fetchMealByCategory()
    ...
    private func updateDataMeal(idMeal: String, scrollProxy: ScrollViewProxy) {
        viewModel.getAPIDetailMealDB(idMeal: idMeal) { success, message in
            if success {
                guard let mealDetail = viewModel.mealDetail else { return }
                viewModel.meal = DetailFollowThemeMealDB().setDetailDataForThemeMealDB(themeMealDB: mealDetail)
            } else {
                
            }
            withAnimation {
                scrollProxy.scrollTo("pageTop", anchor: .top)
            }
            print(success ? "Update Success IDMEAL = \(idMeal)" : "Faild Fetch: \(message)")
        }
    }
}
